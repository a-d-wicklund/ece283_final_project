...
def transpose_range(samples):  # Calculates lowest and highest note.
...
def generate_add_centered_transpose(samples):
    ...
    out_samples = samples
    out_lens = [len(samples), len(samples)]  # Used to check size of out_sample matches out_lens
    for i in range(len(samples)):
        out_sample = np.zeros_like(samples[i])
        out_sample[:, min_note + s:max_note + s] = samples[i][:, min_note:max_note]  # Don't agree with shifting each song differently
        # out_samples = np.append(out_samples, np.expand_dims(out_sample, axis=0), axis=0)  # Array not efficient, creates new array with each append
        out_samples.append(out_sample)
    return out_samples, out_lens  # Returns original with a shifted version appended at the end....?
