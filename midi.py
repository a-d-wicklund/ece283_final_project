...
def find_piano_tracks(mid):
    ...
    for i, track in enumerate(mid.tracks):
        for msg in track:
            if msg.type == 'program_change' and i > 0:
                if 0 <= msg.program < 6:  # To fix, multiple prgram change msgs...
    ...
    for i, track in enumerate(np.array(mid.tracks)[piano_tracks_idxs]):
        ...
        for j, msg in enumerate(track):
            ...
            if msg.type == 'note_on' and msg.velocity != 0:
                note = msg.note - (
                            128 - num_notes) // 2  # Scale down note value (Why?) All notes minus 16, doesn't matter he undoes it when reconstructing.
    ...
    for notes in all_notes:
        ...
        for note in notes:
            ...
            for start, end in notes[note]:
                sample_ix = start // samples_per_measure  # I think this is the measure index?
                while len(samples) <= sample_ix:  # Append measures as needed
                    samples.append(np.zeros((samples_per_measure, num_notes), dtype=np.uint8))
                sample = samples[sample_ix]  # Choose corresponding measure
                start_ix = start - sample_ix * samples_per_measure  # Start index relative to each measure
